...
def generateRecastData(recastScript,runInfo,processFolder):
    """
    Runs the recast script of the Delphes output generated by MG5
    corresponding to the runInfo
    """

    t0 = time.time()

    # Get Delphes file:
    delphesOutput = os.path.join(processFolder,'Events',runInfo['run number'])
    delphesOutput = os.path.join(delphesOutput,'%s_delphes_events.root' %runInfo['run tag'])
    delphesOutput = os.path.abspath(delphesOutput)
    if not os.path.isfile(delphesOutput):
        logger.warning('Could not find Delphes output %s' %delphesOutput)
        return False

    logger.info("Running recast %s on %s" %(recastScript,delphesOutput))
    run = subprocess.Popen('./%s -f %s' %(recastScript,delphesOutput),
                           shell=True,stdout=subprocess.PIPE,
                           stderr=subprocess.PIPE) 
    output,errorMsg= run.communicate()  
    logger.debug('Recast error:\n %s \n' %errorMsg)
    logger.debug('Recast output:\n %s \n' %output)     
    
    logger.info("Finished recasting run in %1.2f min" %((time.time()-t0)/60.))
