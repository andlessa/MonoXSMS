...
Specific details about the event generation for the results shown below are given in [this note](validationNotes.md).


The validation plots for the Vector, Axial, Pseudoscalar and Scalar simplified DM models are shown below. A k-factor is needed in order to better reproduce the CMS analysis results.
...
Below we show cutflows for several benchmarks. We compare the values to the CMS analysis (CMS eff column) with our validation (Recast eff column) in the "Relative differece column".
