...
def vTagging(jetAK8,vList,dataset,drJV=0.2):
    ...
    if abs(jetAK8.Eta) > 2.4:
        return False
    drs = [np.sqrt((jetAK8.Eta-v.Eta)**2 + (jetAK8.Phi-v.Phi)**2) 
                for v in vList]
    if min(drs) > drJV:
        return False

    dr = min(drs)
    v = vList[drs.index(dr)]
    pid = abs(v.PID)
    effHigh = deepAK8effs(jetAK8.PT,pid,dataset,'tight')
    effRnd = np.random.uniform(0.,1.)
    if effRnd < effHigh:
        return 'high-purity'

    effLow = deepAK8effs(jetAK8.PT,pid,dataset,'loose')
    effRnd = np.random.uniform(0.,1.)
    if effRnd < effLow:
        return 'low-purity'
...
def getRecastData(inputFiles,pTcut=150.0,mode='DM+QCDjets'):
    ...
    srCategories = ['monoVhigh','monoVlow','monoJ']
    yields = {sr : {ds : [] for ds in luminosities} for sr in srCategories}
    metAll = {sr : {ds : [] for ds in luminosities} for sr in srCategories}
    totalweightPB = 0.0
    # Keep track of yields for each dataset
    cutListJ = ['Fullsample','Triggeremulation','$p_{T}^{miss}>250$GeV', 'Electronveto',
                'Muonveto', 'Tauveto', 'Bjetveto', 'Photonveto','$\Delta \phi (jet,p_{T}^{miss})>0.5$ rad',
                'LeadingAK4jet$p_{T}>100$GeV', 'LeadingAK4jet$\eta<2.4$','HCALmitigation(jets)',
                'HCALmitigation($\phi^{miss}$)']
    cutListV = cutListJ[:9] + ['LeadingAK8jet$p_{T}>250$GeVand$\eta<2.4$',
                               'HCALmitigation(jets)','HCALmitigation($\phi^{miss}$)']
    cutListVhigh = cutListV[:] + ['LeadingAK8jetDeepAK8tag(high-purity)']
    cutListVlow = cutListV[:] + ['LeadingAK8jetDeepAK8tag(low-purity)']

    cutFlowAll = {'monoJ' : {ds : {cut : 0.0 for cut in cutListJ} for ds in luminosities},
                 'monoVhigh' : {ds : {cut : 0.0 for cut in cutListVhigh} for ds in luminosities},
                 'monoVlow' : {ds : {cut : 0.0 for cut in cutListVlow} for ds in luminosities}
                 }
    ...
    for inputFile in inputFiles:
        ...
        #         missingET = tree.GenMissingET.At(0)  # USE REAL MISSING ET!
            ...
            for ijet in range(jetsAK8.GetEntries()):
                jet = jetsAK8.At(ijet)
                if jet.PT > pTjmin and abs(jet.Eta) < etamax:
            ...
            if not jetAK8List:
                ...
                jetAK8 = jetAK8List[0]                
                vTag = vTagging(jetAK8,vList,useDataSet)
                if vTag == 'high-purity':
                    useSR = 'monoVhigh'
                elif vTag == 'low-purity':
                    useSR = 'monoVlow'
                else:
                    useSR = 'monoJ'
            ...
            cutFlow = cutFlowAll[useSR][useDataSet]
            ...
            if useSR == 'monoVhigh':
                if jetAK8.PT < 250.0 or abs(jetAK8.Eta) > 2.4:  # Dummy cut
                    continue
                cutFlow['LeadingAK8jet$p_{T}>250$GeVand$\eta<2.4$'] += ns
                cutFlow['LeadingAK8jetDeepAK8tag(high-purity)'] += ns
            elif useSR == 'monoVlow':
                if jetAK8.PT < 250.0 or abs(jetAK8.Eta) > 2.4:  # Dummy cut
                    continue
                cutFlow['LeadingAK8jet$p_{T}>250$GeVand$\eta<2.4$'] += ns
                cutFlow['LeadingAK8jetDeepAK8tag(low-purity)'] += ns
            else:
                ## Jet cuts
            ...
            # Store relevant data        
            yields[useSR][useDataSet].append(ns)
            metAll[useSR][useDataSet].append(missingET.MET)  
    ...
    # Create one pickle file for each signal region category (monoV-high, monV-low, monoJ)
    allDataDict = {}
    for sr in srCategories:
        cFlowAll = cutFlowAll[sr]
        mAll = metAll[sr]
        Ys = yields[sr]

        for cutFlow in cFlowAll.values():
        ...
        # Create a dictionary with lists for each dataset/sr category
        ...
        dataDict.update({key : [] for key in list(cFlowAll.values())[0]})
...
if __name__ == "__main__":
    ...
    ap.add_argument('-m', '--mode', required=False,default='DM+QCDjets',type=str,
            help='Mode to classify the type of process (DM+QCDjets, DM+Z(qq) or DM+W(qq))')
    ...
    modelDict = getRecastData(inputFiles,args.pTcut,args.mode)

    # #### Create pandas DataFrame
    for sr in modelDict:
        df = pd.DataFrame.from_dict(modelDict[sr])
        # ### Save DataFrame to pickle file
        print('Saving to',outputFile.replace('.pcl','_%s.pcl' %sr))
        df.to_pickle(outputFile.replace('.pcl','_%s.pcl' %sr))
