...
class ConfigParserExt(RawConfigParser):
    ...
    def expandLoops(self):
        ...
        # Ensure all loop variables have the same length
        lengths = [len(v) for v in varValues]
        if len(set(lengths)) != 1:
            raise ParsingError("All loop variables must have the same length")
        ...
        for idx in range(lengths[0]):
            ...
            for i, (sect, opt) in enumerate(loopVars):
                v = varValues[i][idx]
                newParser.set(sect, opt, str(v))
                logger.debug(f"Setting loop variable {sect}:{opt} to {v}")
