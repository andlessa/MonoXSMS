...
sr_list = ['IM0', 'IM1', 'IM2', 'IM3', 'IM4', 'IM5', 'IM6', 'IM7', 'IM8', 'IM9', 'IM10', 'IM11', 'IM12',
           'EM0', 'EM1', 'EM2', 'EM3', 'EM4', 'EM5', 'EM6', 'EM7', 'EM8', 'EM9', 'EM10', 'EM11', 'EM12']
...
for inputFile, cls_file, saf_file in zip(inputList_pattern, cls_files, saf_files):
    ...
    cutflow_pattern = os.path.dirname(saf_file)+'/atlas_exot_2018_06/Cutflows/*.saf'
    cutflow_files = sorted([file for file in glob.glob(cutflow_pattern)])
    ...
    try:
        ...
        # Get efficiency and statistical error
        w_dict, w2_dict = {}, {}

        for cutflow_file in cutflow_files:
            w_dict[cutflow_file], w2_dict[cutflow_file] = extract_all_cuts_weights(cutflow_file)
        ...
        for i, sr_val in enumerate(sr_list):
            effs_dict[sr_val] = effs[i]
            effs_stat_dict[sr_val] = effs_stat[i]
            ...
            if not line.strip().startswith("#"): 
                ...
                try:
                    ...
                    final_results.append((sr, sigma_exp, sigma_obs,
                                        ...
                                        mMed, mchi, effs_dict[sr], effs_stat_dict[sr]))
