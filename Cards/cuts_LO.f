...
C     CUTS on DM total pT
C-----------------------------

c Mediator pt cut
      do i=nincoming+1,nexternal
         do j=i+1,nexternal
            if ((abs(idup(i,1,1)).eq.52).and.(idup(i,1,1).eq.-idup(j,1,1))) then
              ptDM = sqrt((p(1,i)+p(1,j))**2 + (p(2,i)+p(2,j))**2)
              if (ptDM.lt.150.0) then
                  passcuts = .false.
                  return
              endif
            endif
